...
describe('BookMinter', () => {
    ...
    it('should ASK then BID', async () => {
        ...
        const TSPSettingResult = await SCorderBook.sendNewSession(ACTAdmin.getSender(), {
            value: toNano('0.01'),
            qi: BigInt(Math.floor(Date.now() / 1000)),
            newTradingSessionPriceMin: 1.3 * TSP_DIVIDER, 
            newTradingSessionPriceMax: 1.5 * TSP_DIVIDER, 
        })
        ...
        const TSPSettingResult2 = await SCorderBook.sendNewSession(ACTAdmin.getSender(), {
            ...
            newTradingSessionPriceMax: 1.2 * TSP_DIVIDER, 
        })
